...
if (! $sortfield) $sortfield="cd.date_fin_validite";
if (! $sortorder) $sortorder="ASC";

$filter= 'expired'; //GETPOST("filter");
...
$sql = "SELECT count(p.rowid) as amount_services, c.rowid as cid, c.ref, c.statut as cstatut,";
...
if ($resql)
{
	...
	print_liste_field_titre($langs->trans("amount_expired_services"),"services.php", "amount_services",$param,"","",$sortfield,$sortorder);
	print '<td></td>';
	// Date debut
...
*/
		...
		print '<td>';
		print $obj->amount_services;
		print '</td>';
		
		print '<td>';
		print '<a href="'.DOL_URL_ROOT.'/customerservices/index.php?id='.$obj->socid.'">'.$langs->trans('services').'</a>';
		print '</td>';
		// Start date
/*		if ($mode == "0") {

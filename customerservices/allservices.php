...
$sall=GETPOST('sall');
if(empty($sall))
	header("Location: ".DOL_URL_ROOT."/contrat/services.php?leftmenu=contracts" );
...
/*if($conf->global->MAIN_MODULE_CUSTOMINFO)
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."custominfodet as fd ON cd.rowid = fd.fk_contratdet";
*/
...
if(!empty($sall)){
//	$sql.= " AND ( MATCH (c.ref, p.ref, cd.description, cd.label, c.note_public) AGAINST ('".mysql_real_escape_string($sall)."' IN BOOLEAN MODE) ";
	$sql.= " AND ( cd.label LIKE '%".$sall."%'";
	$sql.= "OR c.note_public LIKE '%".$sall."%'";
	$sql.= "OR cd.description LIKE '%".$sall."%'";
	
	/*if($conf->global->MAIN_MODULE_CUSTOMINFO){
		$sql.= "OR (fd.value LIKE '%".$sall."%' AND fd.value IS NOT NULL)";
	}*/
	$sql.= ") ";
}
...
if ($resql)
{
	...
	while ($i < min($num,$limit))
	{
		...
		//Contratdet description
		print '<td><i>'.$obj->cddesc.'</i></td>';
